...
/*resource "aws_cloudwatch_event_target" "emergency_targets" {
  for_each = local.emergency_event_map

  rule      = aws_cloudwatch_event_rule.ec2_events[each.key].name
  target_id = "${var.instance_name}-${replace(each.key, " ", "-")}-emergency"
  arn       = aws_sns_topic.ec2_state_change.arn
    ...
    input_template = <<TEMPLATE
{
  "instance_id": "<instanceId>",
  "state": "<state>",
  "priority": "${lookup(local.priority_map, each.key)}"
}
TEMPLATE
  }
}
...
resource "aws_cloudwatch_event_target" "normal_targets" {
  for_each = local.normal_event_map

  rule      = aws_cloudwatch_event_rule.ec2_events[each.key].name
  target_id = "${var.instance_name}-${replace(each.key, " ", "-")}-normal"
  arn       = aws_sns_topic.ec2_state_change.arn
}*/
